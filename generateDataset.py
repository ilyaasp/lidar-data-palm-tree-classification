...
for x in range(1, baris, 1): 
    for y in range(1, kolom, 1):
        if ((x+((r*2)))<=baris and (y+((r*2)))<=kolom): # Apakah masih didalam region dimensi gambar?
            if (gr_oil_palm[x+r,y+r]==255):  # Red gak?
                cv.imwrite("dataset/img_rgb0_4500/kelas_sawit/rgb/"+str(temp_inc_oil+586)+".tif", img_rgb[x:(x+((r*2))), y:(y+((r*2)))])
                ...
                temp_inc_oil+=1
            if (gr_non_oil_palm[x+r,y+r]==255): # Blue gak?
                cv.imwrite("dataset/img_rgb0_4500/bukan_sawit/rgb/"+str(temp_inc_non_oil+1210)+".tif", img_rgb[x:(x+((r*2))), y:(y+((r*2)))])
...
cv.imshow('Image RGB', img_rgb[1:60,1:60])
cv.waitKey(0)
